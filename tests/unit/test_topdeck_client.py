...
class TestRequest:
    ...
    def test_401_raises_value_error(self):
        """Test that 401 errors raise ValueError"""
        client = TopDeckClient(api_key="test_key")
        mock_response = Mock()
        ...
        with patch.object(client.session, 'request', return_value=mock_response):
            with pytest.raises(ValueError, match="Invalid TopDeck API key"):
                client._request('GET', '/test')
    
    def test_400_raises_value_error_with_details(self):
        """Test that 400 errors raise ValueError with error details"""
        client = TopDeckClient(api_key="test_key")
        mock_response = Mock()
        ...
        with patch.object(client.session, 'request', return_value=mock_response):
            with pytest.raises(ValueError, match="TopDeck API Bad Request"):
                client._request('POST', '/v2/tournaments', json={'game': 'test'})
    
    def test_400_logs_request_body(self):
        """Test that 400 errors log the request body for debugging"""
        client = TopDeckClient(api_key="test_key")
        mock_response = Mock()
        mock_response.status_code = 400
        ...
        with patch.object(client.session, 'request', return_value=mock_response):
            with patch('src.services.topdeck_client.logger') as mock_logger:
                ...
                # Verify error was logged with request body
                error_calls = [call for call in mock_logger.error.call_args_list 
                             if 'Bad Request' in str(call)]
                assert len(error_calls) > 0, "Error should be logged"
                assert 'Request body' in str(error_calls[0]), "Request body should be logged"
