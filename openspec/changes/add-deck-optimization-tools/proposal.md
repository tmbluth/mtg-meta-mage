...
## What Changes
- Add `optimize_mainboard` MCP tool that analyzes a user's maindeck, identifies non-critical cards (flex spots), and recommends replacements in response to on actual meta decklists from the top N most frequent archetypes
...
- Tools fetch up to 5 recent decklists per archetype from the database to provide concrete card data to the LLM
- Tools provide justifications for recommendations based on actual cards observed in meta decklists (why these cards, why not alternatives)
