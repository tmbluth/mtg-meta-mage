...
-- Archetype groups table for storing unique archetype definitions
-- This table stores unique combinations of (format, main_title, strategy, color_identity).
-- Each unique archetype combination gets one row, eliminating duplication across decklists.
-- Must be defined before decklists table since decklists references it.
-- Fields:
...
CREATE TABLE IF NOT EXISTS decklists (
    ...
    tournament_id TEXT NOT NULL,
    decklist_text TEXT,
    archetype_group_id INTEGER,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (player_id, tournament_id),
    FOREIGN KEY (player_id, tournament_id) REFERENCES players(player_id, tournament_id) ON DELETE CASCADE,
    FOREIGN KEY (archetype_group_id) REFERENCES archetype_groups(archetype_group_id) ON DELETE SET NULL
);
...
-- Archetype classifications table for storing LLM-based classification events
-- This table stores historical classification attempts for decklists.
-- Each row represents one classification event with its metadata (confidence, model, prompt).
-- Multiple classifications can exist per deck (for reclassification tracking).
-- Fields:
--   classification_id: Unique identifier for each classification event (primary key)
--   decklist_id: Foreign key to decklists table (the deck being classified)
--   archetype_group_id: Foreign key to archetype_groups table (the archetype assigned)
...
-- Indexes for archetype_groups table
CREATE INDEX IF NOT EXISTS idx_archetype_groups_format ON archetype_groups(format);

-- Indexes for archetype_classifications table
CREATE INDEX IF NOT EXISTS idx_archetype_classifications_decklist ON archetype_classifications(decklist_id);
CREATE INDEX IF NOT EXISTS idx_archetype_classifications_group ON archetype_classifications(archetype_group_id);
CREATE INDEX IF NOT EXISTS idx_decklists_archetype_group_id ON decklists(archetype_group_id);
