"""
Meta Analytics MCP Tool - FORMAT-LEVEL meta analytics and research.

This module provides MCP tools for format-wide archetype performance and matchup analytics.
Use these tools for GENERAL meta research, not specific deck advice.
"""
...
def get_format_meta_rankings(
    ...
    """
    Get format-wide meta rankings showing which archetypes are most played.
    
    Use this when users ask: "What's the meta like?" or "Top decks in Modern?"
    This is for GENERAL meta analysis, not specific deck advice.
    ...
    current_start, current_end, previous_start, previous_end = _calculate_time_windows(
        current_days, previous_days
    )

    # Fetch data for both periods
    current_archetype_data = _fetch_archetype_data(format, current_start, current_end)
    previous_archetype_data = _fetch_archetype_data(format, previous_start, previous_end)

    current_match_data = _fetch_match_data(format, current_start, current_end)
    previous_match_data = _fetch_match_data(format, previous_start, previous_end)

    # Calculate meta share and win rates
    current_meta = _calculate_meta_share(current_archetype_data)
    previous_meta = _calculate_meta_share(previous_archetype_data)

    current_wins = _calculate_win_rate(current_match_data)
    previous_wins = _calculate_win_rate(previous_match_data)

    # Merge current and previous data
    result = _merge_period_data(current_meta, previous_meta, current_wins, previous_wins)

    # Apply filters
    if color_identity:
        result = _filter_by_color_identity(result, color_identity)
    if strategy:
        result = _filter_by_strategy(result, strategy)

    # Apply grouping
    if group_by == "color_identity":
        result = _group_by_color_identity(result)
    elif group_by == "strategy":
        result = _group_by_strategy(result)
...
@mcp.tool()
def get_format_matchup_stats(
    format: str,
    days: int = 14,
) -> dict:
    """
    Get complete matchup statistics for ALL archetypes in a format.
    
    Use this when users ask: "Show me the full matchup spread" or 
    "What are all the matchups in Modern?"
    This is for GENERAL meta research, not analyzing a specific deck.
    ...
    Returns:
        Dictionary with complete matchup matrix and metadata
    """
    ...
    # Fetch match data
    match_data = _fetch_match_data(format, start_date, end_date)
    ...
    matrix = _calculate_matchup_matrix(match_data)
...
def _calculate_time_windows(
    current_days: int, previous_days: int
...
def _fetch_archetype_data(format: str, start_date: datetime, end_date: datetime) -> pl.DataFrame:
    """
...
def _fetch_match_data(format: str, start_date: datetime, end_date: datetime) -> pl.DataFrame:
    """
...
def _calculate_meta_share(df: pl.DataFrame) -> pl.DataFrame:
    """
...
def _calculate_win_rate(df: pl.DataFrame, min_matches: int = 3) -> pl.DataFrame:
    """
...
def _filter_by_color_identity(df: pl.DataFrame, color_identity: str) -> pl.DataFrame:
    """Filter DataFrame by color identity."""
    return df.filter(pl.col("color_identity") == color_identity)


def _filter_by_strategy(df: pl.DataFrame, strategy: str) -> pl.DataFrame:
    """Filter DataFrame by strategy."""
    return df.filter(pl.col("strategy") == strategy)


def _group_by_color_identity(df: pl.DataFrame) -> pl.DataFrame:
    """
...
def _group_by_strategy(df: pl.DataFrame) -> pl.DataFrame:
    """
...
def _calculate_matchup_matrix(df: pl.DataFrame, min_matches: int = 3) -> dict:
    """
    ...
    for row in matchup_stats.iter_rows(named=True):
        ...
        matrix[player][opponent_archetype] = {
